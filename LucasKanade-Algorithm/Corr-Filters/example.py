...
def animate(i):
    ...
    if i < N:  # If the animation is still running
        ...
        l = 1
        #4.3 calculatioon of 'g' , correlation filters 
        #persevals theorem from notes.!
        
        print(X)
        g = np.linalg.inv(X @ X.T + l * np.eye(X.shape[0])) @ X @ Y
        g = g.reshape((29,45)) 
        
        cor = correlate(img,g)
        print(cor.shape)
        plt.figure()
        plt.title("correlation Lambda = 1" )
        plt.imshow(cor, cmap='gray')
        
        plt.figure()
        plt.title("Filter lambda = 1" )
        plt.imshow(g, cmap='gray')
        
        
        #4.4 Convolution kernel flip
        
        g_  = np.fliplr(g)
        g_ = np.flipud(g_)
        conv_withflip = convolve(img,g)
        plt.figure()
        plt.title("convolution withoutFLip lambda = 1")
        plt.imshow(conv_withflip, cmap='gray')
